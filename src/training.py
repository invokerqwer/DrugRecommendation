...
import torch
import numpy as np
from util import llprint, multi_label_metric, ddi_rate_score, get_n_params,focal_loss
import time
...
def Train(
    ...
    optimizer, log_dir, coef, target_ddi,weighted_bce, EPOCH=50, mu=1e-4,gamma=2e-4
):
    ...
    for epoch in range(EPOCH):
        ...
        for step, input_seq in enumerate(data_train):
            for adm_idx, adm in enumerate(input_seq):
                ...
                result, loss_ddi, vq_loss = model(
                    patient_data=input_seq[:adm_idx + 1],
                    **drug_data,
                    bottleneck = False
                )
                result_cgib, loss_ddi_cgib,KL_Loss,patient_pred_loss,preserve_rate = model(
                    patient_data=input_seq[:adm_idx + 1],
                    **drug_data,
                    bottleneck = True
                )
                sigmoid_res = torch.sigmoid(result)
                sigmoid_cgib = torch.sigmoid(result_cgib)

                loss_bce = binary_cross_entropy_with_logits(result, bce_target)
                if weighted_bce:
                    loss_bce = focal_loss(result, bce_target, alpha=0.25, gamma=2.0)
                    loss_bce_cgib = focal_loss(result_cgib, bce_target, alpha=0.25, gamma=2.0)
                else:
                    loss_bce = binary_cross_entropy_with_logits(result, bce_target)
                    loss_bce_cgib = binary_cross_entropy_with_logits(result_cgib, bce_target)
                
                loss_multi = multilabel_margin_loss(sigmoid_res, multi_target)
                loss_multi_cgib = multilabel_margin_loss(sigmoid_cgib, multi_target)
                ...
                if current_ddi_rate <= target_ddi:
                    ...
                    loss += 0.95 * loss_bce_cgib + 0.05 * loss_multi_cgib
                else:
                    beta = coef * (1 - (current_ddi_rate / target_ddi))
                    beta = min(math.exp(beta), 1)
                    loss = beta * (0.95 * loss_bce + 0.05 * loss_multi) \
                        + (1 - beta) * loss_ddi
                    loss += beta * (0.95 * loss_bce_cgib + 0.05 * loss_multi_cgib) \
                        + (1 - beta) * loss_ddi_cgib
        ...
        ddi_rate, ja, prauc, avg_p, avg_r, avg_f1, avg_med = \
            eval_one_epoch(model, data_train, voc_size, drug_data)
        print('train data ddi: {}, Med: {}, Ja: {}, F1: {}, PRAUC: {}'.format(
        ...
        if epoch >= 5:
            print('eval ddi: {}, Med: {}, Ja: {}, F1: {}, PRAUC: {}'.format(
                np.mean(history['ddi_rate'][-5:]),
                np.mean(history['med'][-5:]),
                np.mean(history['ja'][-5:]),
                np.mean(history['avg_f1'][-5:]),
